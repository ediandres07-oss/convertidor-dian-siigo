...
def safe_float(value):
    """Convierte valores infinitos o NaN a 0"""
...
def safe_round(value, decimals=2):
    """Redondea un valor de forma segura"""
    try:
        val = float(value)
        if val == float('inf') or val == float('-inf') or pd.isna(val):
            return 0.0
...
async def procesar_completo(file: UploadFile = File(...)):
    ...
    df = pd.read_excel(buffer, sheet_name='Empleados')
    ...
    for idx, row in df.iterrows():
        ...
        salario = safe_float(row.get('salario_mensual'))
        dias = safe_float(row.get('dias_laborados'))
        auxilio = safe_float(row.get('auxilio_transporte'))

        if dias <= 0:
            dias = 30

        base = salario + auxilio
        cesantias = safe_float((base * dias) / 360)
        intereses = safe_float((cesantias * 0.12 * dias) / 360)
        prima = safe_float((base * dias) / 360)
        vacaciones = safe_float((salario * dias) / 720)
        total_dev = cesantias + intereses + prima + vacaciones

        empleados.append({
            'documento': doc,
            'nombre': nombre,
            'salario_mensual': safe_round(salario),
            'auxilio_transporte': safe_round(auxilio),
            'dias_laborados': int(dias) if dias > 0 else 30,
            'cesantias': safe_round(cesantias),
            'intereses': safe_round(intereses),
            'prima': safe_round(prima),
            'vacaciones': safe_round(vacaciones),
            'total_devengos': safe_round(total_dev),
            'total_deducciones': 0,
            'neto_pagar': safe_round(total_dev),
        })
...
async def calcular_prima(file: UploadFile = File(...)):
    ...
    for idx, row in df.iterrows():
        ...
        base = salario + auxilio
        valor_prima = safe_float((base * dias) / 360)
        ...
        empleados.append({
            ...
            'auxilio_transporte': safe_round(auxilio),
            'dias_laborados': int(dias) if dias > 0 else 30,
            'valor_prima': safe_round(valor_prima),
        })
    ...
    return {
        "total_empleados": len(empleados),
        "empleados": empleados,
        "total_prima": safe_round(total_prima)
    }
...
async def calcular_vacaciones(file: UploadFile = File(...)):
    ...
    for idx, row in df.iterrows():
        ...
        valor_vacaciones = safe_float((salario * dias) / 720)
        ...
        empleados.append({
            ...
            'salario_mensual': safe_round(salario),
            'dias_laborados': int(dias) if dias > 0 else 30,
            'valor_vacaciones': safe_round(valor_vacaciones),
        })
    ...
    return {
        "total_empleados": len(empleados),
        "empleados": empleados,
        "total_vacaciones": safe_round(total_vacaciones)
    }
